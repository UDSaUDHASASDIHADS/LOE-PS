...
let packetIds = require("./packetIds_23.json");
let switchedPacketIds = (function () {
    let obj = {}
    const name = require("./packetIds_23.json")
    for (x in name) {
        obj[name[x]] = x
    }
    return obj
...
module.exports = {
    ...
    removeKCPHeader(data){
        var buff = Buffer.from(data.slice(28));
        return buff;
    },
    ...
    async objToProtobyName(obj, protoName) {
        try {
            const root = await protobuf.load("./proto/" + protoName + ".proto");
            const testMessage = root.lookup(protoName);
            const message = testMessage.create(obj);
